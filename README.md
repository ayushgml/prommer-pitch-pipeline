...
3. The judge -> draft loop didn't converge. On the live Vercel run, the judge rejected 2 of 10 points on draft 1 and **5 of 8** on the redraft. The writer regenerates every pitch, so it throws away good points and paraphrases the new ones more loosely ("rarely justify" became "don't justify", "worth more" became "substantially more"). **Fixed:** redraft is now repair-only. Accepted points are marked `ok` and frozen, only rejected points go back with the judge's reason plus their quotes, and the judge re-checks only those. Next local run: 3 of 10 rejected, 3 repaired, 1 still unsupported; 3 pitches through instead of 1. One run so far, which is not yet proof it holds.
